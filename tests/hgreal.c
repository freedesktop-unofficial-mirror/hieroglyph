...
TDEF (new)
{
	...
	q = HG_QREAL(1.1);
	fail_unless(q == 0x223f8ccccd, "Unexpected result to create a quark for real: expected: %lx, actual: %lx", 0x223f8ccccd, q);
	f = HG_REAL(q);
	...
	q = HG_QREAL(-1.1);
	fail_unless(q == 0x22bf8ccccd, "Unexpected result to create a quark for integer: expected: %lx, actual: %lx", 0x22bf8ccccd, q);
} TEND
