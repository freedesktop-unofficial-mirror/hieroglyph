...
TDEF (new)
{
	...
	q = HG_QREAL(1.1);
	fail_unless(q == 0xa23f8ccccd, "Unexpected result to create a quark for real: expected: %lx, actual: %lx", 0xa23f8ccccd, q);
	f = HG_REAL(q);
	...
	q = HG_QREAL(-1.1);
	fail_unless(q == 0xa2bf8ccccd, "Unexpected result to create a quark for integer: expected: %lx, actual: %lx", 0xa2bf8ccccd, q);
} TEND
