...
TDEF (get_capsulated_size)
{
	...
	fail_unless(size == sizeof (hg_string_t), "Obtaining the different size: expect: %lx actual: %lx", sizeof (hg_string_t), size);
} TEND
...
TDEF (hg_string_new)
{
	...
	q = hg_string_new(mem, 100, (hg_pointer_t *)&s);
	...
	fail_unless(s->allocated_size == 101, "Unexpected result of the allocated size: expect: %lx actual: %lx", 100, s->allocated_size);
	hg_mem_free(mem, q);
...
*/
	...
	g_free(hieroglyph_test_pop_error());
	q = hg_string_new_with_value(mem, "abc", 3, (hg_pointer_t *)&s);
	fail_unless(q != Qnil, "Unable to create the string object.");
	fail_unless(hg_quark_get_type(q) == HG_TYPE_STRING, "No type information in the quark");
	fail_unless(hg_string_maxlength(s) == 3, "Unexpected result of the allocated size: expect: %lx actual: %lx", 3, s->allocated_size);
	fail_unless(hg_string_length(s) == 3, "Unexpected result of the string size: expect: %lx actual: %lx", 3, s->length);
...
TDEF (hg_string_append_c)
{
	...
	q = hg_string_new(mem, 3, (hg_pointer_t *)&s);
	fail_unless(q != Qnil, "Unable to create the string object.");
...
TDEF (hg_string_erase)
{
	...
	q = hg_string_new_with_value(mem, "abcdefg", 7, (hg_pointer_t *)&s);
	fail_unless(q != Qnil, "Unable to create the string object.");
...
TDEF (hg_string_concat)
{
	...
	q1 = hg_string_new_with_value(mem, "abcd", 4, (hg_pointer_t *)&s1);
	q2 = hg_string_new_with_value(mem, "efgh", 4, (hg_pointer_t *)&s2);
...
TDEF (hg_string_compare)
{
	...
	q1 = hg_string_new_with_value(mem, "abc", 3, (hg_pointer_t *)&s1);
	q2 = hg_string_new_with_value(mem, "abc", 3, (hg_pointer_t *)&s2);
	fail_unless(q1 != Qnil, "Unable to create the string object [1]");
	fail_unless(q2 != Qnil, "Unable to create the string object [2]");
	...
	q2 = hg_string_new_with_value(mem, "abcd", 4, (hg_pointer_t *)&s2);
