...
#define TT(_n_)								\
	...
	fail_unless(q == (0xa300000000 | HG_enc_ ## _n_), "Unexpected result to create a quark for a name: expect: %lx, actual: %lx", (0xa300000000 | HG_enc_ ## _n_), q); \
...
#define TT(_n_)								\
	...
	fail_unless(q == (0xa300000000 | HG_enc_ ## _n_), "Unexpected result to create a quark for the string %s: expect: %lx, actual: %lx", #_n_, (0xa300000000 | HG_enc_ ## _n_), q); \
	p = hg_name_lookup(name, q);					\
	fail_unless(strcmp(p, #_n_) == 0, "Unexpected result to look up the name for %s", #_n_);
